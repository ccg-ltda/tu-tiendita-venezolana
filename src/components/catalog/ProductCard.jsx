...
export function ProductCard({ product, quantity, onQuantity, onAdd }) {
  ...
  return (
    <article className={`card${outOfStock ? ' out-of-stock' : ''}`}>
      <div className='card-img'>{product.image ? <img src={product.image} alt={product.name} loading='lazy' /> : <span className='product-placeholder'><Package size={40} aria-hidden='true' /></span>}<div className='card-sub-badge'>{product.sub}</div>{outOfStock && <div className='stock-badge'>Agotado</div>}</div>
